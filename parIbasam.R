...
# if (sampled==TRUE){
  #dat2<-dat
  #dat <- dat[-which(dat$Population == c("Couesnon","Laita")),] # remove Couesnon
  #dat2$Area[dat2$Population=="Laita"] <- dat$Area[dat$Population=="Laita"] / 2
  #dat2$Area<-sample(dat2$Area)
  #dat2$Distance<-sample(dat2$Distance)
  #dat2 <- dat2[order(dat2$Distance), ]#sort ascending
  #write.csv2(dat2,"data/dataPop2.csv")
# }

#if(shuffled==TRUE) dat <- dat.shuffled
...
#by stage
frates_vec1SW =c(0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1) #c(0, 0.05, 0.07, 0.1)
frates_vecMSW =c(0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1) #c(0, 0.1, 0.15, 0.2)
...
if ( fish.stage==FALSE) {
  frates = c(0, frates_vec[scenarioFishingRate], 0, frates_vec[scenarioFishingRate]) # fishing rates by size
